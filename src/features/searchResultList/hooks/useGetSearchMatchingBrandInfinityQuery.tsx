...
export const useGetSearchMatchingBrandInfinityQuery = () => {
  const { pathname } = useNavigations()

  const [, , , searchParam] = pathname.split("/")
  const decodedSearchParam = decodeURIComponent(searchParam)

  const queryKey = ["searchProductList", "brand", decodedSearchParam]

  const getSearchResultPromiseFn = (pageParam: number) => {
    return searchAPI.getSearchResultMatchingBrand(decodedSearchParam, pageParam)
  }
